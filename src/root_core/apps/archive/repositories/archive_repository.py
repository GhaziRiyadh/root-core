...
from root_core.apps.archive.utils.Image_processor import AsyncImageProcessor
from root_core.apps.archive.utils.utils import get_file_type_by_extension
...
class ArchiveRepository(BaseRepository[Archive]):
    ...
    async def create_or_update_from_path(self, path: Path, id: int | None) -> Archive:
        """
        ...
        async with self.get_session() as session:
            ...
            if id is not None:
                ...
                if file_model:
                    ...
                    file_model.name = path.name
                    file_model.original_path = str(path)
                    file_model.mime_type = mime_type
                    file_model.user_id = user.id if user else None
    ...
    def filter_file_type(self, stmt, value):
        if isinstance(value, list):
            return stmt.where(*[Archive.file_type.ilike(f"%{v}%") for v in value])
        return stmt.where(Archive.file_type.ilike(f"%{value}%"))
