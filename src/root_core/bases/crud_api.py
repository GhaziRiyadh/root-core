...
class CRUDApi(BaseRouter):
    ...
    async def create_item(self, item_data: dict, request: Request):
    ...
    async def bulk_create_items(self, items_data: list, request: Request):
        if not self.create_schema:
            return error_response(
                error_code="SCHEMA_ERROR",
                message="Create schema not provided",
    ...
    # Keep same behavior by setting need_auth=False on decorator.
